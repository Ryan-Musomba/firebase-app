...
import { login, signInWithGoogle, signInWithTwitter, resetPassword } from '../firebase/auth';
import { getUserProfile } from '../firebase/firestore'; // Adjust path as needed
...
function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const updatedUser = { ...userCredential.user, profile };
      setUser(updatedUser);
      
      // Determine navigation based on role
      const userRole = profile?.userType || selectedRole; // Use profile role if exists, else use selectedRole
      switch (userRole) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const userRole = profile?.userType || USER_ROLES.CUSTOMER;
      switch (userRole) {
        case USER_ROLES.STAFF:
    ...
    <div className="max-w-md w-full space-y-8 bg-white/95 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-[#8B4513]/10">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          {activeTab === 'login' ? 'Sign in to access your account' : 'Enter your email to reset your password'}
        </p>
      </div>

      <div className="flex border-b border-[#8B4513]/20">
        <button
      ...
      {activeTab === 'login' && (
        <form onSubmit={handleLogin} className="mt-8 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="appearance-none rounded-t-md relative block w-full pl-10 pr-3 py-3 border border-[#8B4513]/20 placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#8B4513] focus:border-[#8B4513] sm:text-sm"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="appearance-none rounded-b-md relative block w-full pl-10 pr-10 py-3 border border-[#8B4513]/20 placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#8B4513] focus:border-[#8B4513] sm:text-sm"
          ...
          <div className="flex items-center justify-between">
            <Link to="/signup" className="text-sm font-medium text-[#8B4513] hover:text-[#A0522D]">
        ...
        <form onSubmit={handlePasswordReset} className="mt-8 space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="appearance-none rounded-md relative block w-full pl-10 pr-3 py-3 border border-[#8B4513]/20 placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#8B4513] focus:border-[#8B4513] sm:text-sm"
                placeholder="Email address"
                required
              />
            </div>
          ...
          <div className="text-center">
            <button
              type="button"
              onClick={() => setActiveTab('login')}
              className="font-medium text-[#8B4513] hover:text-[#A0522D] transition-colors duration-200"
            >
              Back to login
            </button>
          </div>
